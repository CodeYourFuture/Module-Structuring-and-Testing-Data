...
function penceToPounds(penceString) {
    ...
    penceString = penceString.replaceAll("p", ""); //remove any ps present


    penceString = penceString.replace(/^0+/, ""); //remove any leading 0s 

    penceString = penceString.padStart(3, "0"); // if string has a length<3, pad it with leading 0s until it reaches length 3 (983p =>=> 983, 40p ==> 40 ==> 040)

    return `£${penceString.slice(0, -2)}.${penceString.slice(-2).padEnd(2, "0")}`
    
}

const myPennies = '369p';
