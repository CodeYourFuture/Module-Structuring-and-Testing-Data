...
console.assert(
    ...
    "Test Failed: Expected 2/3 to be true"
);

console.assert(
  isProperFraction(5, 2) === false,
  "Test Failed: Expected 5/2 to be false"
);

try {
    isProperFraction(3, 0);
    console.error("Test Failed: Expected an error for 3/0");
} catch(error) {
    console.assert(error.message === "Denominator cannot be zero", "Test Failed: Wrong error message for 3/0");
}

console.assert(
  isProperFraction(-4, 7) === true,
  "Test Failed: Expected -4/7 to be true"
);

console.assert(
  isProperFraction(3, 3) === false,
  "Test Failed: Expected 3/3 to be false"
);