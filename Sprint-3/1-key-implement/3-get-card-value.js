...
const queenOfCard = getCardValue("Q♦");
assertEquals(queenOfCard, 10);
...
try {
  getCardValue("Z♠");
  console.assert(false, "Expected an error for invalid card rank 'Z♠'");
} catch (error) {
  assertEquals(error.message, "Invalid card rank.");
}
try {
  getCardValue("11♣");
  console.assert(false, "Expected an error for invalid card rank '11♣'");
} catch (error) {
  assertEquals(error.message, "Invalid card rank.");
}
