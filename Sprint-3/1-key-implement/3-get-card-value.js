...
function getCardValue(card) {
  const rank = card.slice(0, -1); // Get the rank part of the card (everything except the last character)
    ...
    if (rank === "A") return 11;
    if(!isNaN(rank)) return parseInt(rank, 10); // Convert numeric ranks to integers
    if (["K", "Q", "J", "10"].includes(rank)) return 10; // Face cards and 10 are worth 10 points
...
try {
  getCardValue("Z♣"); // Invalid card rank
}
catch (error) {
  console.assert(
    error.message === "Invalid card rank.",
    `Expected error message to be "Invalid card rank.", but got "${error.message}"`
  );
}
