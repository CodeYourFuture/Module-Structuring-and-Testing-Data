...
function getAngleType(angle) {
  ...
  else if (angle === 0) return "Zero Angle";
  else if (angle > 0 && angle < 90) return "Acute angle"; // Fixed typo here
  else if (angle > 90 && angle < 180) return "Obtuse angle"; // Corrected angle check
  else if (angle === 180) return "Straight angle";
  else if (angle > 180 && angle < 360) return "Reflex angle"; // Corrected angle range
  else if (angle === 360) return "Complete angle";
}
